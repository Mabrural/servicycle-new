<div class="d-sm-flex align-items-center justify-content-between border-bottom">
    <div>
        ...
        <p class="text-muted">Selamat datang, {{ auth()->user()->name }} - {{ $mitra->business_name ?? 'Bengkel Anda' }}
        </p>
    </div>
    <div>
                            ...
                            <tbody>
                                @foreach ($data['todayQueue'] as $order)
                                    <tr>
                                        ...
                                        <td>{{ $order->vehicle_brand_manual ?? '-' }}
                                            {{ $order->vehicle_model_manual ?? '-' }}</td>
                                        <td>
                                            @php
                                                $statusColors = [
                                                    'waiting' => 'warning',
        ...
        <div class="card">
            <div class="card-body">
                                    ...
                                    <td>
                                        ...
                                        @elseif($order->estimated_cost)
                                            <span class="text-muted">Rp
                                                {{ number_format($order->estimated_cost, 0, ',', '.') }}</span>
                                        @else
                ...
                <div class="text-center mt-3">
                    <a href="{{ route('service-orders.index') }}" class="btn btn-primary text-white">Lihat Semua
                        Order</a>
                </div>
            </div>
        </div>

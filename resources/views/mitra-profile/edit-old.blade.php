{{-- sebelum ada pencarian di maps (walaupun mapsnya gk Lengkap) --}}
                        ...
                        <form action="{{ route('update.mitra', $item->id) }}" method="POST"
                            ...
                            <div class="mb-3">
                                ...
                                <textarea name="address" rows="3"
                                    class="form-control">{{ old('address', $item->address) }}</textarea>
                            </div>
                            ...
                            <div class="row g-3" id="imageGrid">
                                ...
                                @for ($i = 0; $i < 5; $i++) @php $image=$item->images->firstWhere('sort_order', $i);
                                    @endphp

                                    <div class="col-md-2">
                                        <div class="image-card {{ $i === 0 ? 'cover' : '' }}" data-slot="{{ $i }}"
                                            data-mitra="{{ $item->id }}">

                                            @if ($image)
                                            <img src="{{ asset('storage/' . $image->image_path) }}">

                                            <button type="button" class="btn-remove-image" data-id="{{ $image->id }}"
                                                data-slot="{{ $i }}">
